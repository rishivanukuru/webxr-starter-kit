...
class App{
	constructor(){
        ...
        function onSelect() {
            ...
            if ( self.reticle.visible ) {
                ...
                self.ybot.object.position.setFromMatrixPosition( self.reticle.matrix );
    ...
    loadGLTF()
    {
		...
		loader.load(
			...
			function ( gltf ) {
				...
				self.ybot = new Player(options);
                self.ybot.object.visible = true;
				
				self.ybot.action = 'piano';
				const scale = 0.007;
                self.ybot.object.rotation.set(1.57,0,0);
                self.ybot.object.position.set(0,-1,-2);

				self.ybot.object.scale.set(scale, scale, scale); 
                self.scene.add( self.ybot.object ); 
	...
	render( timestamp, frame ) {
        ...
        if (this.ybot) this.ybot.update(dt);
        if (this.ybotdefault) this.ybotdefault.update(dt);
